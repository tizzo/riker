...
class CommandCollection extends CommandBase {
  ...
  runSubCommand(commandName, done) {
    var command = this.getSubCommand(commandName);
    ...
    command.initialize();
    command.configure(this.yargs.argv, function(error) {
      command.run(done);
    });
  }
  ...
  run(done) {
    ...
    if (this.subCommandExists(this.subcommandName)) {
